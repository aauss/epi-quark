...
class Score:
    ...
    The `Scorer` offers epidemiologically meaningful scores and plots
    for these scores given data on case counts of infectious diseases,
    information on cases linked through an outbreak, and signals for outbreaks
    generated by outbreak detection algorithms.
    """

    def __init__(
        self, data: pd.DataFrame, signals:pd.DataFrame, data_labels:list[str], missing_signal_agg="mean"
    ):
        """Builds scorer given data.

        Args:
            data: Case numbers with rows as cells and columns as data labels.
            signals: Signal weights with rows as cells and columns as signal. Signal names must start with 'w_'
            data_labels: Column names of `data` that are the outbreak labels.
            missing_signal_agg: Defaults to "mean". Relevant if there is no information on endemic or non-outbreak cases. All pandas agg strings are valid here
        """
