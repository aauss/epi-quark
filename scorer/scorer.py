...
class Score:
    """Algrithm agnostic scorer for (disease) outbreak detection.

    The `Scorer` offers epidemiologically meaningful scores and supporting
    plots given case count data of infectious diseases,
    information on cases linked through an outbreak, and signals for outbreaks
    generated by outbreak detection algorithms.
    """
    ...
    def __init__(
        ...
        missing_signal_agg="mean",
    ) -> None:
        """Builds scorer given data.

        Args:
            data: Case numbers with rows as data grid cells and columns as data labels and coordinates.
            signals: Signal weights with rows as data grid cells and columns as signal labels and coordinates. 
                Signal names must start with 'w_'
            data_labels: Column names of `data` that are the outbreak labels.
            missing_signal_agg: Defaults to "mean". Relevant if there is no information on endemic or non-outbreak cases. 
                Each string is valid that can be interpreted by pandas.DataFrame.aggregate().
        """
    ...
    def _pad_non_case(self, data: pd.DataFrame) -> pd.DataFrame:
        """Pads case numbers for non_case column if they are missing.

        Args:
            data: DataFrame with case numbers without information on non_cases.

        Returns:
            data DataFrame with padded non_case numbers.
        """
    ...
    def _pad_signals(
        self, signals: pd.DataFrame, aggregation_func: str
    ) -> pd.DataFrame:
        """
    ...
    def plot_map(self, series, title) -> plt.Axes:
        """Plots 2-D grid given a sorted Series.

        This function takes a Series which can contain signals or
        case counts for a data label. Plots is assuming that it
        is sorted as data and signals tables.
        """
    ...
    def eval_df(self) -> pd.DataFrame:
        """Creates DataFrame with comparison of p(d_i | x) and p^(d_i | x)"""
    ...
    def class_based_conf_mat(
        ...
        """Prints confusion matrix and performance metircs for all data labels."""
        if p_thresh is None:
        ...
        for i, label in enumerate(pivot_eval.columns.levels[1]):
            ...
            sns.heatmap(df_cm, annot=True, ax=axes[i]).set(
